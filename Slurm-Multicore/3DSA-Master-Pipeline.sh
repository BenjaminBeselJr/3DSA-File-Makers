#!/bin/bash
set -e

# --- Core Count Handling ---
# If $1 is not set, default to 4 cores.
CORES=${1:-4}

echo "Selected Core Count: $CORES"
echo "---------------------------------------------------------------------"

# Shell and Stats Script Paths
SHELL_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-Binaries-and-Shell-w.slurm"
STATS_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-Shell-Cloud-Stats-Multicore.slurm"
SLAB_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-Variable-Slab-Averages-Multicore.slurm"
TIME_AVG_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-Time-Averaged-Slab-Means-Multicore.slurm"

# VPG & KE Script Paths
KE_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-KE-All-Multicore.slurm"
VPG_BB_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-VPG-and-VPGB-Multicore.slurm"
VPG_DN_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-VPG-DN-Multicore.slurm"
VPG_DL_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-VPG-DL-Multicore.slurm"

# Commented out for future use: Distance Script paths
# NEAREST_QL_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-Nearest-QL.slurm" NOTE: make a multicore core version and replace this path with the real one
# CONNECTED_QL_SLURM="/mnt/stor-pool-01/users/2821011/3DSA-File-Makers/Slurm-Multicore/3DSA-Batch-Connected-Shell-Distance-Multicore.slurm"

# ---------------------------------------------------------------------
# STAGE 1: Root Tasks (No Dependencies)
# ---------------------------------------------------------------------
# Launch independent VPG calculations concurrently
JOB_VPG_BB=$(sbatch --parsable --cpus-per-task=$CORES "$VPG_BB_SLURM")
echo "✅ Root Task Submitted: VPG B and B (Job ID: $JOB_VPG_BB)"

# Corrected variable name alignment to match the diagram
JOB_VPG_DN=$(sbatch --parsable --cpus-per-task=$CORES "$VPG_DN_SLURM")
echo "✅ Root Task Submitted: VPG DN     (Job ID: $JOB_VPG_DN)"

JOB_VPG_DL=$(sbatch --parsable --cpus-per-task=$CORES "$VPG_DL_SLURM")
echo "✅ Root Task Submitted: VPG DL     (Job ID: $JOB_VPG_DL)"

# Launch core pipeline tracking element
JOB_SHELL=$(sbatch --parsable --cpus-per-task=$CORES "$SHELL_SLURM")
echo "✅ Root Task Submitted: Shell Creation (Job ID: $JOB_SHELL)"



# ---------------------------------------------------------------------
# STAGE 2: Tasks dependent on Root Tasks (Cloud Stats & KE)
# ---------------------------------------------------------------------
# Track A: Cloud Stats depends ONLY on Shell Creation
JOB_STATS=$(sbatch --parsable --cpus-per-task=$CORES --dependency=afterok:$JOB_SHELL "$STATS_SLURM")
echo "✅ Stage 2A Queued: Cloud Stats (Job ID: $JOB_STATS, waits for $JOB_SHELL)"

# Track B: Kinetic Energy (Requires Shell Mask + All 3 independent VPG inputs)
JOB_KE=$(sbatch --parsable --cpus-per-task=$CORES --dependency=afterok:$JOB_SHELL:$JOB_VPG_BB:$JOB_VPG_DN:$JOB_VPG_DL "$KE_SLURM")
echo "✅ Stage 2B Queued: Kinetic Energy calculation (Job ID: $JOB_KE, waits for 4 root prerequisites)"

# Optional future tracks (Commented Out)
# JOB_NEAR_QL=$(sbatch --parsable --cpus-per-task=$CORES --dependency=afterok:$JOB_SHELL "$NEAREST_QL_SLURM")
# JOB_CONN_QL=$(sbatch --parsable --cpus-per-task=$CORES --dependency=afterok:$JOB_SHELL "$CONNECTED_QL_SLURM")

# ---------------------------------------------------------------------
# STAGE 3: Slab averages
# ---------------------------------------------------------------------
# This step requires BOTH Cloud Stats and KE to finish successfully
JOB_SLAB=$(sbatch --parsable --cpus-per-task=$CORES --dependency=afterok:$JOB_STATS:$JOB_KE "$SLAB_SLURM")
echo "✅ Stage 3 Queued: Slab Averages (Job ID: $JOB_SLAB, waits for $JOB_STATS and $JOB_KE)"

# ---------------------------------------------------------------------
# STAGE 4: Time Averaged Slab Means
# ---------------------------------------------------------------------
JOB_TIME_AVG=$(sbatch --parsable --cpus-per-task=$CORES --dependency=afterok:$JOB_SLAB "$TIME_AVG_SLURM")
echo "✅ Stage 4 Queued: Time Averaged Slab Means (Job ID: $JOB_TIME_AVG, waits for $JOB_SLAB)"

echo "---------------------------------------------------------------------"
echo "Pipeline successfully linked! Track progress using 'squeue -u \$USER'"