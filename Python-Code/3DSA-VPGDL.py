...
ds_w = xr.open_dataset(path_ds_w, decode_times=False,chunks={'time': 1}).rename({'zh':'z'}).interp(z=ds_p.z)
...
for t in range(num_times):
    print(f"\n--- (DL) Processing Timestep {t}/{num_times - 1} ---")
    #slice inputs
    ...
    # =====================================================================
    print(f"\n--- (DL) Running Spectral & Staggered Verification of pi DL at Timestep {t}/{num_times - 1} ---")
    ...
    pi_DL_hat = np.fft.rfft2(pi_DL_numpy, axes=(1, 2))
    sigma_3d = sigma_numpy[:, np.newaxis, np.newaxis]
    horizontal_laplacian_hat = -sigma_3d * k_sq[np.newaxis, :, :] * pi_DL_hat
    ...
    print(f"(DL) Pure Spectral-Aligned Spatial Correlation for Timestep {t}/{num_times - 1}: {correlation:.6f}")

    if np.allclose(test_lhs_spectral, truth_interior, rtol=1e-2, atol=1e-2):
        print(f"(DL) Verification of Timestep {t}/{num_times - 1}: SUCCESS - Solver approximately matches mathematical formulation")
    else:
        print(f"(DL) Verification of Timestep {t}/{num_times - 1}: FAILURE - Discrepancies found.")

    #VPG DL
    dpi_dz_xr = pi_DL_xr.differentiate(coord="z")
    vpg_DL_numpy = -c_pd * th.values[:, np.newaxis, np.newaxis] * dpi_dz_xr.compute().values

    #Computation complete
    print("DL Computation complete")

    #Exporting
    print(f"(DL) Exporting timestep {t} datasets...")

    open_files["pi_dl.nc"].variables["pi_dl"][t, :, :, :] = pi_DL_numpy
    open_files["vpg_dl.nc"].variables["vpg_dl"][t, :, :, :] = vpg_DL_numpy
